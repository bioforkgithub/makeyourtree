...
it, build on it commercially; keep the copyright notice and accept that it comes with no
warranty. Full text in [`LICENSES/MIT.txt`](LICENSES/MIT.txt).
...
Parts of the MakeYourTree source code, its test suite and its documentation were written
with the assistance of a large language model (Anthropic Claude), used as a coding and
writing tool under the direction of the author.

The author specified the architecture and the interface contracts, directed the
implementation, reviewed the code, and is responsible for the whole of the software. Every
number quoted in the documentation was produced by running the program and can be
reproduced with the scripts in [`tools/`](tools/). None was generated or estimated by a
language model. The limitations recorded in [`docs/STATUS.md`](docs/STATUS.md) were
established by running the software, not inferred.

No artificial intelligence system is named as an author or holds any copyright here. Such a
system cannot accept responsibility for the work, nor answer correspondence about it, so it
does not meet the criteria for authorship. Under the Indian Copyright Act 1957 the author of
a computer-generated work is "the person who causes the work to be created" (§2(d)(vi)), and
US and EU practice likewise require human authorship. The copyright follows the human
authorship: the architecture, the specifications the code was written against, the review,
the testing, and the decisions about what the software should do. All of these are Manish
Prakash Victor's.

The same disclosure is recorded in [`legal/AUTHORSHIP.md`](legal/AUTHORSHIP.md) §3, and it
accompanies any publication describing the software.

---

## Independence

MakeYourTree is an independent, clean-room implementation. **It is not affiliated with,
endorsed by, or derived from any other phylogenetics package.** No code, markup, help
text, icon, template file or example dataset from any other tree-visualisation product was
consulted or copied. File formats and algorithms are implemented from their published
specifications and from the primary literature, each cited in the module that implements
it.

The audit trail is in [`legal/CLEANROOM.md`](legal/CLEANROOM.md) and
[`legal/PROVENANCE.md`](legal/PROVENANCE.md).
...
MakeYourTree shares the problem domain with these tools and none of their code.
